    ...
    <div class="grid grid-cols-2 pb-2 border-b border-slate-200">
        ...
        <div class="text-end">
            <x-button type="button" sm squared secondary icon="plus"
                      wire:click="$emit('openModal','super-admin.projects.networks.team-add', [{{json_encode($network->id)}}])"
            />
        </div>
    </div>
    @foreach($network->teams as $team)
        <form>
            <div class="grid grid-cols-2 py-2 border-b border-slate-100">
                <div class="pt-2 pl-2 text-sm text-slate-600 font-bold uppercase">{{ $team->team_name }}</div>
                <div class="text-end content-end items-end align-middle">
                    <x-button
                        type="button"
                        wire:click="$emit('openModal','super-admin.projects.networks.team-delete', [{{json_encode($network->id)}},{{json_encode($team->id)}}])"
                        icon="trash" sm negative/>
                </div>
            </div>
        </form>
    @endforeach
